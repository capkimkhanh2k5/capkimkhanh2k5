<!-- ========================================== -->
<!-- 🎨 HEADER SECTION -->
<!-- ========================================== -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&amp;color=0:7aa2f7,50:bb9af7,100:7dcfff&amp;height=280&amp;section=header&amp;text=C%C3%A1p%20Kim%20Kh%C3%A1nh&amp;fontSize=75&amp;fontAlignY=30&amp;fontColor=ffffff&amp;desc=Backend%20Engineer%20%7C%20Full-Stack%20Developer%20%7C%20AI%20%26amp;%20IoT%20Enthusiast&amp;descAlignY=52&amp;descSize=22&amp;animation=fadeIn" />
</div>

<div align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&amp;weight=700&amp;size=24&amp;duration=3000&amp;pause=1000&amp;color=7AA2F7&amp;center=true&amp;vCenter=true&amp;multiline=true&amp;repeat=true&amp;width=750&amp;height=80&amp;lines=%F0%9F%8E%93+Software+Engineering+%40+DUT+%C4%90%C3%A0+N%E1%BA%B5ng;%F0%9F%91%A8%E2%80%8D%F0%9F%92%BB+Spring+Boot+%7C+Django+%7C+Next.js+%7C+Flutter+%7C+IoT" alt="Typing SVG" />
  </a>
</div>

<div align="center">
  
  [![GitHub](https://img.shields.io/badge/GitHub-capkimkhanh2k5-1a1b26?style=for-the-badge&logo=github&logoColor=7aa2f7)](https://github.com/capkimkhanh2k5)
  [![Facebook](https://img.shields.io/badge/Facebook-capkhanhs-1a1b26?style=for-the-badge&logo=facebook&logoColor=7aa2f7)](https://fb.com/capkhanhs)
  [![Location](https://img.shields.io/badge/%F0%9F%93%8D_%C4%90%C3%A0_N%E1%BA%B5ng,_Vi%E1%BB%87t_Nam-1a1b26?style=for-the-badge)](https://github.com/capkimkhanh2k5)
  [![Repos](https://img.shields.io/badge/Public_Repos-13-1a1b26?style=for-the-badge&logo=git&logoColor=7aa2f7)](https://github.com/capkimkhanh2k5?tab=repositories)
  
</div>

<br/>

<!-- ========================================== -->
<!-- ABOUT ME -->
<!-- ========================================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/People%20with%20professions/Man%20Technologist%20Light%20Skin%20Tone.png" width="30" /> About Me

> *"Code is like humor. When you have to explain it, it's bad."* — Cory House

I'm **Cáp Kim Khánh** — a 3rd-year **Software Engineering** student at **Da Nang University of Science and Technology (DUT)**. I specialize in **Backend Engineering** with hands-on experience building distributed systems, RESTful APIs, real-time communication, and AI/IoT integration. I'm also capable of **Full-Stack Development** with modern frontend frameworks and cross-platform mobile apps.

<table>
<tr>
<td width="50%" valign="top">

### 🎯 Core Expertise
- ⚙️ **Backend Architecture:** Design & build RESTful APIs with **Spring Boot** & **Django REST Framework**
- 🔐 **Security:** JWT Authentication, Spring Security, OAuth2
- 🔄 **Real-time Systems:** WebSocket (Django Channels, Socket.io), SSE
- 🤖 **AI Integration:** Object Detection (YOLOv8), Google Gemini AI, MobiNet V3
- 📱 **Mobile Development:** Flutter cross-platform apps
- 🔌 **IoT & Embedded:** Arduino/ESP32 firmware programming

</td>
<td width="50%" valign="top">

### 🚀 Currently Working On
- 🔭 Building **ECOSORT** — IoT Smart Waste Classification with AI + Spring Boot + Flutter
- 💼 Developing **JOBIO** — Job Platform with Django, WebSocket real-time, Gemini AI chatbot
- 🌐 Personal Portfolio with **Next.js 14**, Three.js, Framer Motion
- 📚 Studying **System Design**, **Microservices Architecture**, **Cloud Computing**
- 🏗️ Exploring **Docker**, **CI/CD Pipelines**, **Scalable Backend Systems**

</td>
</tr>
</table>

---

<!-- ========================================== -->
<!-- TECH STACK -->
<!-- ========================================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Hammer%20and%20Wrench.png" width="30" /> Tech Stack & Technologies

> All languages, frameworks, and tools gathered from real GitHub projects.

### 💻 Programming Languages
<div align="center">

| Language | Level | Used In |
|:---:|:---:|:---|
| <img src="https://skillicons.dev/icons?i=java&amp;theme=dark" width="40"/> **Java** | ⭐⭐⭐⭐⭐ | Spring Boot, JSP/Servlet, Network Programming |
| <img src="https://skillicons.dev/icons?i=python&amp;theme=dark" width="40"/> **Python** | ⭐⭐⭐⭐⭐ | Django, AI/ML, Data Science, Signal Processing |
| <img src="https://skillicons.dev/icons?i=ts&amp;theme=dark" width="40"/> **TypeScript** | ⭐⭐⭐⭐ | Next.js, React, Portfolio |
| <img src="https://skillicons.dev/icons?i=js&amp;theme=dark" width="40"/> **JavaScript** | ⭐⭐⭐⭐ | Frontend, Node.js, Full-stack apps |
| <img src="https://skillicons.dev/icons?i=cpp&amp;theme=dark" width="40"/> **C++** | ⭐⭐⭐⭐ | Arduino/ESP32, Algorithms, Microcontrollers |
| <img src="https://skillicons.dev/icons?i=c&amp;theme=dark" width="40"/> **C** | ⭐⭐⭐ | Embedded Systems, IoT firmware |
| <img src="https://skillicons.dev/icons?i=dart&amp;theme=dark" width="40"/> **Dart** | ⭐⭐⭐ | Flutter mobile apps |
| <img src="https://skillicons.dev/icons?i=kotlin&amp;theme=dark" width="40"/> **Kotlin** | ⭐⭐ | Android native modules |
| <img src="https://skillicons.dev/icons?i=swift&amp;theme=dark" width="40"/> **Swift** | ⭐⭐ | iOS native modules |
| <img src="https://skillicons.dev/icons?i=r&amp;theme=dark" width="40"/> **R** | ⭐⭐ | Data Science, Statistical Analysis |

</div>

<br/>

### ⚙️ Backend Frameworks & Libraries
<div align="center">
  <img src="https://skillicons.dev/icons?i=spring,django,nodejs,express&amp;theme=dark" />
</div>

<div align="center">

| Framework | Details |
|:---:|:---|
| **Spring Boot 3.x** | Spring Web, Spring Security, Spring Validation, Spring Data JPA, Swagger/OpenAPI (Springdoc) |
| **Django 5.x** | Django REST Framework, Django Channels, Daphne (ASGI), Django Filter, DRF-Spectacular |
| **JSP / Servlet** | Java EE Web Apps, MVC Pattern |
| **Node.js** | Express.js, Backend APIs |

</div>

<br/>

### 🎨 Frontend & UI Frameworks
<div align="center">
  <img src="https://skillicons.dev/icons?i=nextjs,react,tailwind,sass,html,css&amp;theme=dark" />
</div>

<div align="center">

| Framework | Details |
|:---:|:---|
| **Next.js 14** | App Router, SSR/SSG, API Routes, next-themes |
| **React 18** | Hooks, Context, Component Architecture |
| **Tailwind CSS** | Utility-first, tailwind-merge, tailwindcss-animate |
| **Radix UI** | Dialog, Dropdown, Tabs, Toast, Tooltip, Popover, ScrollArea |
| **Framer Motion** | Page transitions, scroll animations, variants |
| **GSAP** | Advanced scroll-triggered animations, @gsap/react |
| **Three.js / Spline** | 3D graphics, interactive 3D scenes |
| **SCSS / Sass** | Modular stylesheets, mixins, variables |

</div>

<br/>

### 📱 Mobile Development  
<div align="center">
  <img src="https://skillicons.dev/icons?i=flutter,dart,kotlin,swift&amp;theme=dark" />
</div>

<div align="center">

| Technology | Details |
|:---:|:---|
| **Flutter** | Cross-platform mobile (Android + iOS), Widget Architecture |
| **Kotlin** | Android native modules integration |
| **Swift** | iOS native modules integration |

</div>

<br/>

### 🗄️ Databases & Caching
<div align="center">
  <img src="https://skillicons.dev/icons?i=postgres,mongodb,redis,sqlite,firebase&amp;theme=dark" />
</div>

<div align="center">

| Database | Details |
|:---:|:---|
| **PostgreSQL** | Primary RDBMS, psycopg2, complex queries |
| **MongoDB** | NoSQL document store, pymongo, dnspython |
| **Redis** | Caching (django-redis), Pub/Sub, Channels backend |
| **SQLite** | Development & lightweight storage |
| **Firebase** | Firestore, Realtime Database, Firebase Admin SDK, Authentication |

</div>

<br/>

### 🤖 AI / Machine Learning & Data Science
<div align="center">
  <img src="https://skillicons.dev/icons?i=python,opencv,tensorflow&amp;theme=dark" />
</div>

<div align="center">

| Technology | Details |
|:---:|:---|
| **YOLOv8** | Real-time Object Detection (Smart Waste Classification) |
| **OpenCV** | Computer Vision, Image Processing |
| **Google Gemini AI** | Generative AI integration (google-genai SDK) |
| **Jupyter Notebook** | Data Analysis, Model Training, Visualization |
| **NumPy / Pandas** | Data manipulation, Scientific computing |
| **Scikit-learn** | Machine Learning algorithms, Classification |
| **Cython** | Performance optimization for Python |

</div>

<br/>

### 🔌 IoT & Embedded Systems
<div align="center">
  <img src="https://skillicons.dev/icons?i=arduino,cpp,c&amp;theme=dark" />
</div>

<div align="center">

| Technology | Details |
|:---:|:---|
| **Arduino / ESP32** | Firmware development, Sensor integration |
| **C / C++** | Low-level embedded programming |
| **CMake** | Build system for embedded projects |
| **MQTT / Serial** | IoT communication protocols |

</div>

<br/>

### 🔧 DevOps, Tools & Cloud
<div align="center">
  <img src="https://skillicons.dev/icons?i=docker,git,github,postman,linux,bash,maven,vscode&amp;theme=dark" />
</div>

<div align="center">

| Tool | Details |
|:---:|:---|
| **Docker** | Containerization, Docker Compose, multi-service orchestration |
| **Git & GitHub** | Version control, branching strategies, collaboration |
| **Postman** | API testing, collection management, automated tests |
| **Maven** | Java project build & dependency management |
| **Swagger / OpenAPI** | Springdoc OpenAPI, DRF-Spectacular, API documentation |
| **ESLint / Ruff** | Code linting (JavaScript/TypeScript & Python) |
| **Linux / Bash** | Server management, shell scripting |
| **Cloudinary** | Cloud media storage & transformation |
| **Firebase Cloud** | BaaS, Authentication, Cloud Functions |

</div>

<br/>

### 🔄 Real-time & Communication
<div align="center">

| Technology | Details |
|:---:|:---|
| **WebSocket** | Django Channels, Socket.io (Client & Server) |
| **Server-Sent Events** | django-eventstream, real-time notifications |
| **Daphne (ASGI)** | Async server for Django Channels |
| **channels-redis** | Redis-backed channel layer |

</div>

<br/>

### 📦 Notable Libraries & Utilities
<div align="center">

| Library | Type | Description |
|:---:|:---:|:---|
| **Lombok** | Java | Reduce boilerplate code |
| **Pydantic** | Python | Data validation & serialization |
| **SimpleJWT** | Python | JWT Authentication for Django |
| **Zod** | TypeScript | Runtime schema validation |
| **Framer Motion** | React | Animation library |
| **Three.js** | JavaScript | 3D WebGL rendering |
| **Lenis** | JavaScript | Smooth scrolling |
| **Resend** | Node.js | Email service |
| **WeasyPrint** | Python | HTML/CSS to PDF generation |
| **Pillow** | Python | Image processing |

</div>

---

<!-- ========================================== -->
<!-- FEATURED PROJECTS -->
<!-- ========================================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Travel%20and%20places/Rocket.png" width="30" /> Featured Projects

<div align="center">

<table>
<tr>
<td width="50%" valign="top">

### 🌿 ECOSORT — Smart Waste Classification
> IoT smart waste classification system using AI

**Tech Stack:**
- `Spring Boot 3.4` · `Spring Security` · `Firebase`
- `YOLOv8` · `Python` · `OpenCV`
- `Flutter` (Dart) · Cross-platform Mobile
- `Arduino/ESP32` · `C++` · Sensors
- `Swagger` · `Postman` · `Maven`

[![Repo](https://img.shields.io/badge/GitHub-PBL5-7aa2f7?style=flat-square&logo=github)](https://github.com/capkimkhanh2k5/PBL5)

</td>
<td width="50%" valign="top">

### 💼 JOBIO — Job Platform
> Recruitment platform with real-time chat & AI chatbot

**Tech Stack:**
- `Django 5.2` · `DRF` · `PostgreSQL` · `MongoDB`
- `Django Channels` · `WebSocket` · `Redis`
- `Google Gemini AI` · `Cloudinary`
- `Docker` · `Docker Compose`
- `DRF-Spectacular` · `JWT Auth`

[![Repo](https://img.shields.io/badge/GitHub-JOBIO-7aa2f7?style=flat-square&logo=github)](https://github.com/capkimkhanh2k5/JOBIO)

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🌐 Personal Portfolio
> Personal portfolio website with 3D animations

**Tech Stack:**
- `Next.js 14` · `React 18` · `TypeScript`
- `Tailwind CSS` · `SCSS` · `Radix UI`
- `Three.js` · `Spline` · `GSAP`
- `Framer Motion` · `Lenis`
- `Socket.io` · `Resend`

[![Repo](https://img.shields.io/badge/GitHub-My__Portfolio-7aa2f7?style=flat-square&logo=github)](https://github.com/capkimkhanh2k5/My_Portfolio)

</td>
<td width="50%" valign="top">

### � SixDegreeOfSeparation — Graph Theory
> Six Degrees of Separation algorithm using BFS/Graph traversal

**Tech Stack:**
- `C++` · `Python`
- `Graph Theory` · `BFS Algorithm`
- `Data Structures`

[![Repo](https://img.shields.io/badge/GitHub-SixDegreeOfSeparation-7aa2f7?style=flat-square&logo=github)](https://github.com/capkimkhanh2k5/SixDegreeOfSeparation)

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 📡 PBL4 — AI for SAGSINs
> AI for Collaborative Communication in SAGSINs

**Tech Stack:**
- `Python` · `JavaScript`
- `AI/ML Algorithms`
- `Docker` · `Shell`

[![Repo](https://img.shields.io/badge/GitHub-PBL4__SAGGINs-7aa2f7?style=flat-square&logo=github)](https://github.com/capkimkhanh2k5/PBL4_SAGGINs)

</td>
<td width="50%" valign="top">

### 🔗 More Projects
> Academic & research projects

- 📊 **KhoaHocDuLieu** — Data Science (Python, R, Jupyter)
- 🧠 **TriTueNhanTao** — AI Algorithms (Python, Jupyter)
- 🌐 **LTM** — Network Programming (Java Socket)
- 🎛️ **ViDieuKhien** — Microcontrollers (C++)
- 📐 **Math-Information-Tools** — Math Computing (C++, Python)
- � **CONVERT_FILE** — File Conversion (JSP, Servlet, Java EE)
- �🔊 **XLTHS** — Signal Processing (Python)

</td>
</tr>
</table>

</div>

---

<!-- ========================================== -->
<!-- GITHUB ANALYTICS -->
<!-- ========================================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Bar%20Chart.png" width="30" /> GitHub Analytics

### 🏆 Profile Trophies
<div align="center">
  <img src="https://github-trophies.vercel.app/?username=capkimkhanh2k5&amp;theme=tokyonight&amp;margin-w=15&amp;margin-h=15&amp;column=7&amp;no-frame=true&amp;no-bg=true" />
</div>

<br/>

### 📈 Stats & Streak
<div align="center">
  <img src="https://github-readme-stats-sigma-five.vercel.app/api?username=capkimkhanh2k5&amp;show_icons=true&amp;theme=tokyonight&amp;hide_border=true&amp;title_color=7aa2f7&amp;icon_color=7aa2f7&amp;text_color=c0caf5&amp;bg_color=1a1b26&amp;count_private=true&amp;include_all_commits=true" alt="GitHub Stats" width="48%" />
  <img src="https://streak-stats.demolab.com/?user=capkimkhanh2k5&amp;theme=tokyonight&amp;hide_border=true&amp;ring=7aa2f7&amp;fire=bb9af7&amp;currStreakLabel=7aa2f7&amp;currStreakNum=c0caf5&amp;sideNums=c0caf5&amp;sideLabels=c0caf5&amp;background=1a1b26" alt="GitHub Streak" width="48%" />
</div>

<br/>

### 💻 Language Distribution
<div align="center">
  <img src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=capkimkhanh2k5&amp;layout=donut-vertical&amp;theme=tokyonight&amp;hide_border=true&amp;title_color=7aa2f7&amp;text_color=c0caf5&amp;bg_color=1a1b26&amp;langs_count=12" alt="Top Languages" width="40%" />
</div>

<br/>

### 📉 Contribution Graph
<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=capkimkhanh2k5&amp;theme=tokyo-night&amp;bg_color=1a1b26&amp;color=7aa2f7&amp;line=bb9af7&amp;point=7dcfff&amp;hide_border=true&amp;area=true&amp;area_color=7aa2f7" alt="Contribution Graph" width="100%" />
</div>

<br/>

### 🐍 Contribution Snake
<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/capkimkhanh2k5/capkimkhanh2k5/output/github-contribution-grid-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/capkimkhanh2k5/capkimkhanh2k5/output/github-contribution-grid-snake.svg" />
    <img alt="Snake animation" src="https://raw.githubusercontent.com/capkimkhanh2k5/capkimkhanh2k5/output/github-contribution-grid-snake-dark.svg" />
  </picture>
</div>

---

<!-- ========================================== -->
<!-- CONTACT -->
<!-- ========================================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Incoming%20Envelope.png" width="30" /> Contact

<div align="center">

[![Facebook](https://img.shields.io/badge/Facebook-capkhanhs-1877F2?style=for-the-badge&logo=facebook&logoColor=white)](https://fb.com/capkhanhs)
[![GitHub](https://img.shields.io/badge/GitHub-capkimkhanh2k5-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/capkimkhanh2k5)

</div>

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=capkimkhanh2k5&amp;style=for-the-badge&amp;color=7aa2f7&amp;label=PROFILE+VIEWS" alt="Profile Views" />
</div>

---

<!-- ========================================== -->
<!-- FOOTER -->
<!-- ========================================== -->

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&amp;color=0:7aa2f7,50:bb9af7,100:7dcfff&amp;height=120&amp;section=footer" />
</div>

<div align="center">
  <b>"Any fool can write code that a computer can understand.<br/>Good programmers write code that humans can understand."</b>
  <br/>
  <sub>— Martin Fowler</sub>
</div>
